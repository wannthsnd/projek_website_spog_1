...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white mb-1">Edit Permohonan</h1>
            <p class="text-gray-800 dark:text-gray-200 font-extrabold">Perbarui data permohonan #{{ $permit->id }}</p>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('permohonan.update', $permit->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 border-2 border-gray-200 dark:border-gray-700">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Email -->
            <div class="space-y-2">
                <label class="block text-sm font-extrabold text-gray-900 dark:text-white">
                    <i class="bi bi-envelope text-primary-600 mr-2"></i>Email
                </label>
                <input type="email" name="email" value="{{ old('email', $permit->email) }}"
                    class="w-full px-5 py-4 bg-gray-50 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:border-primary-500 transition-all text-gray-900 dark:text-white font-extrabold"
                    required>
                @error('email')
            ...
            <div class="space-y-2 md:col-span-2">
                ...
                <input type="date" name="application_date" value="{{ old('application_date', $permit->application_date) }}"
                    class="w-full px-5 py-4 bg-gray-50 dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:border-primary-500 transition-all text-gray-900 dark:text-white font-extrabold"
                    required>
        ...
        <div class="mt-8 border-t-2 border-gray-200 dark:border-gray-700 pt-8">
            <h3 class="text-2xl font-extrabold text-gray-900 dark:text-white mb-6 flex items-center gap-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 border-2 border-gray-300 dark:border-gray-600 hover:border-primary-400 dark:hover:border-primary-500 transition-all">
                    <label class="block text-sm font-extrabold text-gray-900 dark:text-white mb-2">
                        Dokumen {{ $num }}: {{ $label }}
                    </label>
                    ...
                    <div class="mb-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border-2 border-green-300 dark:border-green-700">
                        <p class="text-xs font-extrabold text-green-800 dark:text-green-400">
                    ...
                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-2 font-extrabold">Format: PDF (Maks. 10MB)</p>
                    @error("document_{$num}")
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
